...
function initializeFirebase() {
    try {
        ...
        // Check if config exists (handle case where firebase-config.js might not load)
        ...
        // Silently fail - will use local storage instead
        return false;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        // Initialize Firebase first (will fail gracefully if not configured)
        initializeFirebase();
        ...
        if (firebaseInitialized) {
            loadSharedData().then(() => {
                ...
                // If Firebase load fails, use local storage
        ...
        console.log('Initialization error, using local storage only');
        loadData();
        initializeUI();
        setupEventListeners();
        updateProgress();
