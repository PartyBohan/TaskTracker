...
document.addEventListener('DOMContentLoaded', () => {
    // Setup image error handlers first
...
function setupImageErrorHandlers() {
    ...
    if (headerIcon1) {
        headerIcon1.addEventListener('error', function() {
            this.style.display = 'none';
            const emoji = document.getElementById('emoji-fallback-1');
            if (emoji) emoji.style.display = 'inline';
        });
    }
    
    if (headerIcon2) {
        headerIcon2.addEventListener('error', function() {
            this.style.display = 'none';
            const emoji = document.getElementById('emoji-fallback-2');
            if (emoji) emoji.style.display = 'inline';
        });
    }
    
    if (smileImg) {
        smileImg.addEventListener('error', function() {
            this.style.display = 'none';
            const emoji = document.getElementById('emoji-fallback-smile');
            if (emoji) emoji.style.display = 'inline';
        });
    }
}
    ...
    if (todayTasks.length === 0) {
        tasksList.innerHTML = `
            <li class="empty-state">
                <img src="images/labubu-empty.png" alt="Labubu" class="labubu-empty-img" id="empty-img">
                <span class="labubu-emoji-fallback" id="empty-emoji" style="display:none;">🐰</span>
                <p>No tasks for today! Add some tasks to get started 🐰</p>
            </li>`;
        // Handle image error
        const emptyImg = document.getElementById('empty-img');
        if (emptyImg) {
            emptyImg.addEventListener('error', function() {
                this.style.display = 'none';
