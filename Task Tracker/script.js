...
function renderTasks() {
    ...
    if (todayTasks.length === 0) {
        tasksList.innerHTML = `
            <li class="empty-state">
                <img src="images/labubu-empty.png" alt="Labubu" class="labubu-empty-img" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
                <span class="labubu-emoji-fallback" style="display:none;">🐰</span>
    ...
    tasksList.innerHTML = todayTasks.map(task => {
        ...
        return `
            <li class="task-item ${isCompletedToday ? 'completed' : ''}">
                <input type="checkbox" 
                       class="task-checkbox" 
    ...
    routineList.innerHTML = routineTasks.map(task => {
        ...
        return `
            <li class="routine-task-item">
                <span class="routine-task-text">${task.text}</span>
                <span class="routine-task-time">🕐 ${formattedTime}</span>
                <button class="delete-routine-btn" onclick="deleteRoutineTask(${task.id})">×</button>
            </li>
        `;
    }).join('');
}
